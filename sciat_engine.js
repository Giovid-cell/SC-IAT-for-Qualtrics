// sciat_engine.js — motore SC-IAT generalizzato (derivato da anna_tosato/anna_sciat_ext.js)
// Generato/mantenuto via SC-IAT Builder.
// Novita': makeControlledTrials gestisce conteggi e numeri di stimoli ARBITRARI;
//          tasti (leftKey/rightKey) e timeout di risposta (responseTimeout) sovrascrivibili da config.
// https://cdn.jsdelivr.net/gh/Giovid-cell/SC-IAT-for-Qualtrics/sciat_engine.js
...
define(['pipAPI','pipScorer','underscore'], function(APIConstructor, Scorer, _) {
	...
	function stiatExtension(options)
	{
		...
		/**
		 * Creates a trial array with controlled stimulus repetition, for ANY count and ANY number of stimuli.
		 * Each stimulus appears floor(count / n) times; the remaining (count % n) trials are assigned
		 * preferentially to the stimuli in repeatedWords, then to the remaining stimuli in order.
		 * Backward compatible (count=10,n=5 -> each 2x; count=7,n=5 -> each 1x + 2 repeats).
		 * @param {number} count
		 * @param {Array} mediaArray - media objects {word:'..'} or {image:'..'}
		 * @param {Array} repeatedWords - stimuli (word/image) favoured for the extra trials
		 * @returns {Array}
		 */
		function makeControlledTrials(count, mediaArray, repeatedWords) {
		    var trials = [];
		    var n = mediaArray.length;
		    if (n === 0 || count <= 0) return trials;
		    repeatedWords = repeatedWords || [];

		    function idOf(m) { return (m && m.word !== undefined) ? m.word : (m && m.image !== undefined ? m.image : m); }

		    var base = Math.floor(count / n);
		    for (var pass = 0; pass < base; pass++) {
		        for (var i = 0; i < n; i++) trials.push(mediaArray[i]);
		    }

		    var rem = count - (base * n);
		    var extras = [];
		    for (var i = 0; i < n && extras.length < rem; i++) {
		        if (repeatedWords.indexOf(idOf(mediaArray[i])) !== -1) extras.push(mediaArray[i]);
		    }
		    for (var i = 0; i < n && extras.length < rem; i++) {
		        if (repeatedWords.indexOf(idOf(mediaArray[i])) === -1) extras.push(mediaArray[i]);
		    }
		    var k = 0;
		    while (extras.length < rem) { extras.push(mediaArray[k % n]); k++; }
		    trials = trials.concat(extras);
		...
		API.addTrialSets('sort', {
		    ...
		    input: [
		        ...
		        {handle: 'left', on: 'keypressed', key: piCurrent.leftKey, isExclusive:true},
		        {handle: 'right', on: 'keypressed', key: piCurrent.rightKey, isExclusive:true},
		        {handle: 'timeout', on: 'timeout', duration: piCurrent.responseTimeout, isExclusive: true}
		    ],
